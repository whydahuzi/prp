...
function moving_while_videos
    % Create a figure window
    hFig = figure('Position', [500, 200, 1100, 1000], 'Name', 'Boids Simulation', 'NumberTitle', 'off');
    movegui(hFig, 'center');
    
    % Create sliders for adjusting the target point A coordinates
    uicontrol('Style', 'text', 'Position', [200, 150, 100, 20], 'String', 'Target X');
    hSliderX = uicontrol('Style', 'slider', 'Position', [190, 140, 200, 20], 'Min', -50, 'Max', 150, 'Value', 50);
    uicontrol('Style', 'text', 'Position', [250, 200, 100, 20], 'String', 'Target Y');
    hSliderY = uicontrol('Style', 'slider', 'Position', [240, 190, 200, 20], 'Min', -50, 'Max', 150, 'Value', 50);
    
    % Initialize boids
    numBoids = 50;
    positions_ground = rand(numBoids, 2) * 100; 
    positions_ground(:, 3) = 0; % Z-axis for ground boids
    velocities_ground = randn(numBoids, 3) / 10;

    positions_sky = rand(numBoids, 2) * 100; 
    ...
    borderDistance = 10; % Adjust as needed
    ...
    while ishandle(hFig)
        target_x = get(hSliderX, 'Value');
        target_y = get(hSliderY, 'Value');
        target_position = [target_x, target_y, 5]; % Target point is at z = 5 (midway between ground and sky)
        ...
        for i = 1:numBoids
            ...
            v6_ground = targetFollowing(positions_ground(i, :), target_position); % Target following

            velocities_ground(i, :) = velocities_ground(i, :) + v1_ground + v2_ground + v3_ground + v4_ground + v5_ground + v6_ground;
            ...
            v6_sky = targetFollowing(positions_sky(i, :), target_position); % Target following

            velocities_sky(i, :) = velocities_sky(i, :) + v1_sky + v2_sky + v3_sky + v4_sky + v5_sky + v6_sky;
    ...
    function v6 = targetFollowing(position, target)
        % Calculate the vector towards the target
        v6 = (target - position) * 0.2; % Adjust the factor as needed to control the strength of target following
    end
